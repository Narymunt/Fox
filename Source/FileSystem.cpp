...
long CFileSystem::Search(char name[])
{
  ...
  for (i = 0; i < filesys_header.number_files; i++)
	...
	seeklong += sizeof(filesys_indexblock);	// pomylka w offsetach ? 
	fseek(fptr, -seeklong, SEEK_END);
...
long CFileSystem::GetLokation(char name[])
{
  ...
  return filesys_indexblock.lokation;	// zwroc lokalizacje pliku w archiwum
}
...
long CFileSystem::GetOriginalLength(char name[])
{
  ...
  return filesys_indexblock.length;		// zwroc orginalna dlugosc 
...
long CFileSystem::GetLength(char name[])
{
  ...
  return filesys_indexblock.length;		// zwraca dlugosc
...
short CFileSystem::GetCompressionMethod(char name[])
{
  ...
  return filesys_indexblock.compression;		// zwroc metode kompresji
}
...
char *CFileSystem::pGetFileName(long index)
{
  ...
  return (char*)filesys_indexblock.name;	// zwraca wskaznik do lancucha z nazwa
