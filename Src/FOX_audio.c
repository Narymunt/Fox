...
// - generowanie sampli
// - reverb
// - przy strcmp rozroznianie na duze i male literki [001]
// - mixowanie liniowe 
...
int FOX_AudioInit(const char *driver_name)
{
	...
	if ( audio == NULL ) 
	{
		if ( driver_name != NULL ) 
		{
			...
			// [001] tutaj na strcmp, moze byc problem ze wzgledu na duze i male litery
...
int FOX_OpenAudio(FOX_AudioSpec *desired, FOX_AudioSpec *obtained)
{
	...
	if ( ! current_audio ) 
	{
		if ( (FOX_InitSubSystem(FOX_INIT_AUDIO) < 0) ||
		     (current_audio == NULL) ) 
		{
			return(-1);
		}
	...
	
	{
		...
		else 
		{
			...
			if ( audio->convert.needed ) 
			{
				...
				audio->convert.buf =(Uint8 *)FOX_AllocAudioMem(
...
FOX_audiostatus FOX_GetAudioStatus(void)
{
	...
	if ( audio && audio->enabled ) 
	{
		if ( audio->paused ) 
		{
			status = FOX_AUDIO_PAUSED;
		} 
		else 
		{
			status = FOX_AUDIO_PLAYING;
		}
	}
	return(status);
}
...
void FOX_LockAudio (void)
{
	...
	if ( audio ) 
	{
		if ( audio->thread && (FOX_ThreadID() == audio->threadid) ) 
		{
			return;
		}
		FOX_mutexP(audio->mixer_lock);
...
void FOX_AudioQuit(void)
{
	...
	if ( audio ) 
	{
		...
		if ( audio->thread != NULL ) 
		{
			FOX_WaitThread(audio->thread, NULL);
		}
		
		if ( audio->mixer_lock != NULL ) 
		{
			FOX_DestroyMutex(audio->mixer_lock);
		}
		
		if ( audio->fake_stream != NULL ) 
		{
			FOX_FreeAudioMem(audio->fake_stream);
		}
		
		if ( audio->convert.needed ) 
		{
			FOX_FreeAudioMem(audio->convert.buf);
