...
FOX_Surface * FOX_SetVideoMode (int width, int height, int bpp, Uint32 flags)
{
  ...
  fprintf(stderr,
	...
	if ( (video->screen->flags & FOX_OPENGL) &&
...
int FOX_GL_LoadLibrary(const char *path)
{
	FOX_VideoDevice *video = current_video;
	FOX_VideoDevice *this = current_video;
	int retval;
	...
	if ( video && video->GL_LoadLibrary ) {
		...
		FOX_SetError("No dynamic GL support in video driver");
	}
	return(retval);
}

void *FOX_GL_GetProcAddress(const char* proc)
{
	FOX_VideoDevice *video = current_video;
	FOX_VideoDevice *this = current_video;
	void *func;
	...
	if ( video->GL_GetProcAddress ) {
		if ( video->gl_config.driver_loaded ) {
			...
			FOX_SetError("No GL driver has been loaded");
		}
	} else {
		FOX_SetError("No dynamic GL support in video driver");
	}
...
int FOX_GL_SetAttribute( FOX_GLattr attr, int value )
{
	int retval;
	FOX_VideoDevice *video = current_video;
	...
	switch (attr) {
		...
		case FOX_GL_STENCIL_SIZE:
			...
			break;
		default:
			FOX_SetError("Unknown OpenGL attribute");
			retval = -1;
			break;
	}
	return(retval);
...
int FOX_GL_GetAttribute(FOX_GLattr attr, int* value)
{
	int retval = -1;
	FOX_VideoDevice* video = current_video;
	FOX_VideoDevice* this = current_video;
	...
	if ( video->GL_GetAttribute ) {
		...
		*value = 0;
		FOX_SetError("GL_GetAttribute not supported");
	}
	return retval;
}

/* Perform a GL buffer swap on the current GL context */
void FOX_GL_SwapBuffers(void)
{
	FOX_VideoDevice *video = current_video;
	FOX_VideoDevice *this = current_video;
...
{
	FOX_GL_Lock();
 	FOX_GL_UpdateRects(numrects, rects);
	FOX_GL_Unlock();
}

/* Update rects without state setting and changing (the caller is responsible for it) */
void FOX_GL_UpdateRects(int numrects, FOX_Rect *rects)
{
#ifdef HAVE_OPENGL
	FOX_VideoDevice *this = current_video;
	FOX_Rect update, tmp;
