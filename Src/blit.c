...
//================================================================================================
// TODO:
// - logowanie, jakiego blit uzywamy 
// - usunac blitowanie rle 
// - error do logowania i do konsoli
			...
			FOX_Surface *dst, FOX_Rect *dstrect)
{
	...
	if ( dst->flags & (FOX_HWSURFACE|FOX_ASYNCBLIT) ) 
	{
		...
		if ( video->LockHWSurface(this, dst) < 0 ) okay = 0;
		else dst_locked = 1;
	}
	...
	if ( src->flags & (FOX_HWSURFACE|FOX_ASYNCBLIT) ) 
	{
		...
		if ( video->LockHWSurface(this, src) < 0 ) okay = 0;
		else src_locked = 1;
	}
...
int FOX_CalculateBlit(FOX_Surface *surface)
{
	...
	if ((surface->flags & FOX_RLEACCEL)==FOX_RLEACCEL) FOX_UnRLESurface(surface, 1);
	...
	if ((surface->flags&FOX_SRCALPHA)&&
		(surface->format->alpha!=FOX_ALPHA_OPAQUE||surface->format->Amask) ) 
        blit_index |= 2;
	...
	if ( surface->map->sw_blit == NULL ) surface->map->sw_blit = FOX_SoftBlit;
