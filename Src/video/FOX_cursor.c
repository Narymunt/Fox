...
void FOX_CursorQuit(void)
{
	if ( FOX_cursor != NULL ) 
	{
		...
		if ( FOX_cursor != FOX_defcursor ) 
		{
			FOX_FreeCursor(FOX_cursor);
		}
...
int FOX_CursorInit(Uint32 multithreaded)
{
	...
	if ( multithreaded ) 
	{
		FOX_cursorlock = FOX_CreateMutex();
	}
					...
					int w, int h, int hot_x, int hot_y)
{
	...
	{
		FOX_SetError("Cursor hot spot doesn't lie within cursor");
		return(NULL);
	}
...
void FOX_DrawCursorNoLock(FOX_Surface *screen)
{
	...
	{ int w, h, screenbpp;
	  ...
	  if ( (screen == FOX_VideoSurface) ||
	          FORMAT_EQUAL(screen->format, FOX_VideoSurface->format) ) {
	...
	if ( (area.x == 0) && (area.w == FOX_cursor->area.w) ) {
		FOX_DrawCursorFast(screen, &area);
	} else {
		FOX_DrawCursorSlow(screen, &area);
	}
}
...
void FOX_DrawCursor(FOX_Surface *screen)
{
	...
	if ( (screen == FOX_VideoSurface) &&
	     ((screen->flags & FOX_HWSURFACE) != FOX_HWSURFACE) ) {
		FOX_VideoDevice *video = current_video;
		FOX_VideoDevice *this  = current_video;
		FOX_Rect area;
