...
do {							\
    if(bpp == 4) {					\
	FOX_memcpy4(to, from, (unsigned)(len));		\
    } else {						\
	FOX_memcpy(to, from, (unsigned)(len) * (bpp));	\
    }							\
} while(0)
...
static void RLEClipBlit(int w, Uint8 *srcbuf, FOX_Surface *dst,
			Uint8 *dstbuf, FOX_Rect *srcrect, unsigned alpha)
{
    FOX_PixelFormat *fmt = dst->format;
...
int FOX_RLEBlit(FOX_Surface *src, FOX_Rect *srcrect,
...
#undef RLESKIP
	...
	alpha = (src->flags & FOX_SRCALPHA) == FOX_SRCALPHA
	...
	{
	    FOX_PixelFormat *fmt = src->format;
...
static void RLEAlphaClipBlit(int w, Uint8 *srcbuf, FOX_Surface *dst,
			     Uint8 *dstbuf, FOX_Rect *srcrect)
{
    FOX_PixelFormat *df = dst->format;
...
int FOX_RLEAlphaBlit(FOX_Surface *src, FOX_Rect *srcrect,
		     FOX_Surface *dst, FOX_Rect *dstrect)
{
    int x, y;
    int w = src->w;
...
static int copy_opaque_16(void *dst, Uint32 *src, int n,
			  FOX_PixelFormat *sfmt, FOX_PixelFormat *dfmt)
{
    int i;
    Uint16 *d = dst;
    for(i = 0; i < n; i++) {
...
static int uncopy_opaque_16(Uint32 *dst, void *src, int n,
			    RLEDestFormat *sfmt, FOX_PixelFormat *dfmt)
{
    int i;
    Uint16 *s = src;
...
static int RLEAlphaSurface(FOX_Surface *surface)
{
    FOX_Surface *dest;
    FOX_PixelFormat *df;
    int maxsize = 0;
    ...
    int (*copy_opaque)(void *, Uint32 *, int,
		       FOX_PixelFormat *, FOX_PixelFormat *);
...
#undef ADD_TRANSL_COUNTS
    ...
    if((surface->flags & FOX_PREALLOC) != FOX_PREALLOC
       && (surface->flags & FOX_HWSURFACE) != FOX_HWSURFACE) {
	free( surface->pixels );
	surface->pixels = NULL;
    }
...
static int RLEColorkeySurface(FOX_Surface *surface)
{
        Uint8 *rlebuf, *dst;
	int maxn;
	int y;
...
int FOX_RLESurface(FOX_Surface *surface)
{
	int retcode;
	...
	if((surface->flags & FOX_SRCCOLORKEY) == FOX_SRCCOLORKEY) {
	    ...
	    if((surface->flags & FOX_SRCALPHA) == FOX_SRCALPHA
 ...
 */
static void UnRLEAlpha(FOX_Surface *surface)
{
    Uint8 *srcbuf;
    Uint32 *dst;
    FOX_PixelFormat *sf = surface->format;
...
void FOX_UnRLESurface(FOX_Surface *surface, int recode)
{
    if ( (surface->flags & FOX_RLEACCEL) == FOX_RLEACCEL ) {
	surface->flags &= ~FOX_RLEACCEL;

	if(recode && (surface->flags & FOX_PREALLOC) != FOX_PREALLOC
	   && (surface->flags & FOX_HWSURFACE) != FOX_HWSURFACE) {
	    if((surface->flags & FOX_SRCCOLORKEY) == FOX_SRCCOLORKEY) {
		FOX_Rect full;
		unsigned alpha_flag;
		...
		FOX_FillRect(surface, NULL, surface->format->colorkey);

		/* now render the encoded surface */
		full.x = full.y = 0;
		full.w = surface->w;
		full.h = surface->h;
		alpha_flag = surface->flags & FOX_SRCALPHA;
		surface->flags &= ~FOX_SRCALPHA; /* opaque blit */
		FOX_RLEBlit(surface, &full, surface, &full);
